...
This is a Cloudflare Worker that runs cinaworker on top of [OpenClaw](https://github.com/openclaw/openclaw) in a Cloudflare Sandbox container. It provides:
...
| `OPENCLAW_GATEWAY_TOKEN` | `--token` flag | Mapped from `CINAWORKER_GATEWAY_TOKEN` |
...
R2 is mounted via s3fs at `/data/cinaworker`. Important gotchas:
...
- **Never delete R2 data**: The mount directory `/data/cinaworker` IS the R2 bucket. Running `rm -rf /data/cinaworker/*` will DELETE your backup data. Always check mount status before any destructive operations.
